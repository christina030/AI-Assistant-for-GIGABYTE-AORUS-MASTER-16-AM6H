...
| 檢索演算法 | Top-2 命中率 | 平均 TTFT (s) | 平均 TPS (tokens/s) | 表現 |
| ------------- | ------------- | ------------- | ------------- | ------------- |
| Vector (向量檢索) | 71.4 % (10/14) | 0.051 | 77.17 | 具備語意優勢：在硬體規格查詢的場景中，使用者的自然語言提問（如「看電影音效好嗎」）未必會精準對應到網頁上的專業規格用詞（如「Dolby Vision」）。Vector 檢索能較好地捕捉隱含的語義關聯，因此命中率最高。 |
| BM25 (關鍵字檢索) | 57.1 % (8/14) | 0.041 | 78.25 | 純字詞比對在缺乏精確關鍵字時容易錯失正確文本，導致命中率相對較低。 |
| Hybrid (混合檢索) | 57.1 % (8/14) | 0.034 | 77.56 | 在此次測試中，Hybrid 混合檢索的表現僅與 BM25 持平，並未發揮出「截長補短」的優勢。這可能表示 BM25 的雜訊干擾了排序，未來可針對兩種演算法的分數權重（Alpha 值）進行進一步的調優與正規化。 |
